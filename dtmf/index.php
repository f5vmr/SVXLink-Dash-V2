    ...
    <center>
        <fieldset style = "border:#3083b8 2px groove;box-shadow:5px 5px 20px #999; background-color:#f1f1f1; width:555px;margin-top:15px;margin-left:0px;margin-right:5px;font-size:13px;border-top-left-radius: 10px; border-top-right-radius: 10px;border-bottom-left-radius: 10px; border-bottom-right-radius: 10px;">
            <div style = "padding:0px;width:550px;background-image: linear-gradient(to bottom, #e9e9e9 50%, #bcbaba 100%);border-radius: 10px;-moz-border-radius:10px;-webkit-border-radius:10px;border: 1px solid LightGrey;margin-left:0px; margin-right:0px;margin-top:4px;margin-bottom:0px;line-height:1.6;white-space:normal;">
                <center>
                    <h1 id="dtmf_info" style = "color:#00aee8;font: 18pt arial, sans-serif;font-weight:bold; text-shadow: 0.25px 0.25px gray;">Набор DTMF</h1>
...
if (isset($_POST['button1'])) {
    shell_exec('echo "91235#" > /var/run/svxlink/dtmf_svx');
}

if (isset($_POST['button2'])) {
    shell_exec('echo "912350#" > /var/run/svxlink/dtmf_svx');
}

if (isset($_POST['button3'])) {
    shell_exec('echo "9123561#" > /var/run/svxlink/dtmf_svx');
}

if (isset($_POST['button4'])) {
    shell_exec('echo "910#" > /var/run/svxlink/dtmf_svx');
}

if (isset($_POST['button5'])) {
    shell_exec('echo "919050#" > /var/run/svxlink/dtmf_svx');
}

if (isset($_POST['button6'])) {
    shell_exec('echo "9143136#" > /var/run/svxlink/dtmf_svx');
}

if (isset($_POST['button7'])) {
    shell_exec('echo "9149720#" > /var/run/svxlink/dtmf_svx');
}
...
if (isset($_POST['button11'])) {
    shell_exec('echo "2# 453582#" > /var/run/svxlink/dtmf_svx');
}

if (isset($_POST['button12'])) {
    shell_exec('echo "2# 871657## " > /var/run/svxlink/dtmf_svx');
}
...
{
    shell_exec('echo "A" > /var/run/svxlink/dtmf_svx');
 //   echo '<pre><h1><center><p style="color: #454545; ">Send DTMF: #</center></h1></p></pre$    }
}
if (isset($_POST['buttonBB']))
{
    shell_exec('echo "B" > /var/run/svxlink/dtmf_svx');
 //   echo '<pre><h1><center><p style="color: #454545; ">Send DTMF: #</center></h1></p></pre$    }
}

if (isset($_POST['buttonCC']))
{
    shell_exec('echo "C" > /var/run/svxlink/dtmf_svx');
 //   echo '<pre><h1><center><p style="color: #454545; ">Send DTMF: #</center></h1></p></pre$    }
}

if (isset($_POST['buttonDD']))
{
    shell_exec('echo "D" > /var/run/svxlink/dtmf_svx');
 //   echo '<pre><h1><center><p style="color: #454545; ">Send DTMF: #</center></h1></p></pre$    }
}
