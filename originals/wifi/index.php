...
function isWifiAvailable() {
    $output = shell_exec("sudo -n nmcli device status");
    return strpos($output, 'wlan0') !== false;
}

// Function to check if connected to Wi-Fi
function isWifiConnected() {
    $output = shell_exec("sudo -n nmcli -t -f DEVICE,STATE device | grep '^wlan0:connected'");
    return !empty(trim($output));
}

// Function to check if wired ethernet is connected
function isWiredConnected() {
    $output = shell_exec("sudo -n nmcli -t -f DEVICE,STATE device | grep '^eth0:connected'");
    return !empty(trim($output));
}
