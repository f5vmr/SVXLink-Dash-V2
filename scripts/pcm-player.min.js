...
this.ws = new WebSocket("ws://" + document.location.hostname + ":" + t);
...
function updateListenerCountDisplay(count) {
    ...
    // Subtract 1 so your own connection doesn’t show
    const visibleCount = Math.max(0, count - 1);

    el.textContent = visibleCount > 0 ? `Listeners: ${visibleCount}` : '';
}
...
//    const el = document.getElementById('listenerCount');
//    // Subtract 1 so your own connection doesn’t show
//    const visibleCount = count > 1 ? count - 1 : 0;
//    if (visibleCount > 0) {
//        el.textContent = `Listeners: ${visibleCount}`;
//    } else {
//        el.textContent = '';
//    }
//}
